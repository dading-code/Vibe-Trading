...
export function Correlation() {
  ...
  const [codes, setCodes] = useState("BTC-USDT,ETH-USDT,SP500,AAPL");
  ...
  const compute = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("correlation.computeError"));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col gap-4 border rounded-lg p-4">
        <div className="flex flex-col gap-1.5">
          ...
          <p className="text-xs text-muted-foreground">
            {t("correlation.assetCodesDescription")}
          </p>
        </div>

        <div className="flex flex-wrap gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium">{t("correlation.windowDays")}</label>
